...
DEFAULT_REVIEW_SERVER = "review.stxnext.pl"
...
def add_auth_handler():
    try:
        auth_config = open(CONFIG_FILE)
    except:
        print('Nie znaleziono pliku konfiguracyjnego')
    
    config = ConfigParser.ConfigParser()
    config.readfp(auth_config)
    try:
        url = config.get('review_server', 'server_url')
        username = config.get('auth', 'username')
        passwd = config.get('auth', 'password')
    except:
    	print('Brak informacji do logowania w pliku konfiguracyjnym')
	return None
    mgr = urllib2.HTTPPasswordMgrWithDefaultRealm()
    mgr.add_password(None, url, username, passwd)
    return urllib2.HTTPBasicAuthHandler(mgr)
...
class HttpRpcServer(AbstractRpcServer):
  ...
  def _Authenticate(self, login_url="/accounts/login/"):
    ...
    try:
      self.opener.add_handler(add_auth_handler())
      response = self.opener.open(req)
      #response_body = response.read()
      #response_dict = dict(x.split("=")
      #                     for x in response_body.split("\n") if x)
...
VCSES = ['hg', 'git', 'svn', 'cvs']
ADDITIONAL_INFO = ['--issue', ]


def prepare_config():
    """Get arguments for upload from config file.  """
    ...
    for x in config.sections():
        for key, val in config.items(x):
            if key in options: result.extend([options[key], val])
    #"Assume yes" parameter
    result.append('-y')
    return result


def process_args():
    """Prepares version control system 
       command to be run in subprocess.
    """

    if len(sys.argv) < 2: return 'no_vcs'
    ags = sys.argv
    if ags[1].startswith('--vcs='):
        vcs = ags[1]
        ags.remove(vcs)
        vcs = vcs.split('=')[1]
    else: vcs = None
...
def get_issue_number():
    if '--issue' in sys.argv:
        ind = sys.argv.index('--issue')
        issue_num = sys.argv[ind + 1]
        sys.argv.pop(ind + 1)
        sys.argv.pop(ind)
        return issue_num
...
def get_vcs():
    if '--vcs' in sys.argv:
        ind = sys.argv.index('--vcs')
        vcs_name = sys.argv[ind + 1]
        sys.argv.pop(ind + 1)
        sys.argv.pop(ind)
        return vcs_name
...
def get_credentials_from_config():
    """Gets username and password from the config file.
    """
    try: 
        config_file = open(CONFIG_FILE)
    except:
...
def get_diff_options():
    """ Gets additional diff options 
        from the config file and returns them as a list
    """
    result = []
    diff_options = {
                'lines' : '-U',
    ...
    config = ConfigParser.ConfigParser()
    config.readfp(config_file)
    for key, val in config.items('diff'):
        if key in diff_options:
            result.append(''.join([diff_options[key], val]))
    return result


def review_board_only(cmd):
    if '__review' in sys.argv:
        rev = True
        sys.argv.remove('__review')
    else:
        rev = False
    return rev or (cmd == 'no_vcs')
